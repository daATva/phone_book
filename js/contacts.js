...
function filterContacts() {
    ...
    const copiedContacts = JSON.parse(JSON.stringify(contacts));
    

    contacts = copiedContacts.filter(contact => {
      if (searchText === '') {
        displayContacts();
      }
...
// Разобраться с копиями , чтобы было правильное возвращение
// Баг на сердечке
// Поменять на поле действия + сделать проверку на инпут и каждый раз на при нажатиее 
// На бекс пейс или делит , проверять на буквы и выводить , что подходит и записывать
// А когда будет 0 символом , то высирать весь список
// Я решал это задание , кеш обновился и оно пропало 
